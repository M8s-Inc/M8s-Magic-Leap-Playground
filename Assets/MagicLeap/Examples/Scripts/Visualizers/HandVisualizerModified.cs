...
namespace MagicLeap
{
    ...
    /// </summary>
    public class HandVisualizerModified : MonoBehaviour
    {
        ...
        private GameObject Left_IndexFinger_Raycast = null;
        private GameObject Right_IndexFinger_Raycast = null;
        ...
        void Start()
        {
            if (_center == null)
            {
                Debug.LogError("Error: HandVisualizerModified._center is not set, disabling script.");
                enabled = false;
                return;
            }
            ...
            if (!result.IsOk)
            {
                Debug.LogErrorFormat("Error: HandVisualizerModified failed calling Start on MLHandTrackingStarterKit, disabling script. Reason: {0}", result);
                enabled = false;
                return;
            }
            #endif
        ...
        void Update()
        {
            ...
            if (MLHandTracking.IsStarted)
            {
                ...
                if (_handType == MLHandTracking.HandType.Left)
                {
                    Left_IndexFinger_Raycast.transform.position = _indexFinger[1].localPosition;
                    Left_IndexFinger_Raycast.transform.LookAt(_indexFinger[2].localPosition);

                }
                else
                {
                    Right_IndexFinger_Raycast.transform.position = _indexFinger[1].localPosition;
                    Right_IndexFinger_Raycast.transform.LookAt(_indexFinger[2].localPosition);            
                }
        ...
        Left_IndexFinger_Raycast = GameObject.Find("LeftHand-Raycast");
        Right_IndexFinger_Raycast = GameObject.Find("RightHand-Raycast");
    }
